...
//var_dump($pass);exit;
 ...
 if($cek > 0){
  echo " <script>document.location.href='?modul=kategori&status=11'";
    ...
    $sql = mysql_query("INSERT INTO kategori (nama_kategori,kategori_seo) VALUES ('".$nama."','".$slug."') ");
    if($sql){
...
$slug  = seotitle($nama);
//var_dump($pass);exit;
$sql = mysql_query("UPDATE kategori SET nama_kategori='".$nama."',kategori_seo = '".$slug."' WHERE id ='".$id."'");
if($sql){
        echo " <script>document.location.href='?modul=kategori&status=2'";
        echo "</script>";
    }            
